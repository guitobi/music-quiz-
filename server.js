...
io.on('connection', socket => {
    ...
    socket.on('rejoin-room', ({ userNickname, roomCode }) => {
        ...
        if (rooms[socket.room].roomTimeoutTimer) clearTimeout(rooms[socket.room].roomTimeoutTimer);
    ...
    socket.on('disconnect', () => {
        ...
        if (!rooms[socket.room].gameIsStarted && Object.keys(rooms[socket.room].players).length === 0) {
